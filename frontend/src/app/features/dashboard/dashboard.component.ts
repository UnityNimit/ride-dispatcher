...
import { DashboardStats, DriverTripStats } from '../../core/models/dashboard.model';
...
export class DashboardComponent implements OnInit {
  ...
  readonly driverAnalytics = signal<DriverTripStats[]>([]);
  readonly analyticsColumns = ['driverName', 'completedTrips', 'totalRevenue', 'averageFare'];
  ...
  readonly activeRiderTrip = signal<Trip | null>(null);
  ...
  ngOnInit(): void {
    ...
    if (role === 'ADMIN') {
      ...
      this.dashboardService.getTripsPerDriver().subscribe({
        next: (rows) => this.driverAnalytics.set(rows),
        error: () => this.driverAnalytics.set([])
      });
      return;
    }

    if (role === 'RIDER') {
      this.tripService.getMyTrips(0, 10).subscribe({
        next: (page) => {
          this.recentTrips.set(page.content.slice(0, 5));
          this.activeRiderTrip.set(
            page.content.find((t) => ['REQUESTED', 'ACCEPTED', 'ARRIVED', 'IN_PROGRESS'].includes(t.status)) ?? null
          );
          this.loading.set(false);
        },
