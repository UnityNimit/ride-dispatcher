...
export class TripDetailComponent implements OnInit, OnDestroy {
  ...
  readonly driverProfileId = signal<number | null>(null);
  ...
  ngOnInit(): void {
    ...
    if (this.currentRole() === 'DRIVER') {
      this.driverService.getMyDriverProfile().subscribe({
        next: (profile) => this.driverProfileId.set(profile.id),
        error: () => this.driverProfileId.set(null)
      });
    }
  ...
  canCancel(trip: Trip): boolean {
    ...
    if (role === 'RIDER' && userId != null && trip.riderId === userId) {
      return true;
    }
    if (role === 'DRIVER' && trip.driverId === this.driverProfileId() && trip.status !== 'IN_PROGRESS') {
      return true;
    }
    return false;
  }
